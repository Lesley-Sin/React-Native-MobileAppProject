...
import type { ConversationDetails } from "./Interfaces/IConversationDetails";
import type { MessageDetails } from "./Interfaces/MessageDetails";
import type { IMessagingState } from "./Interfaces/IMessagingState";
import type { ISubscription } from "./Interfaces/ISupscription";
import type { ConversationMessagesDetails } from "./Interfaces/ConversationMessagesDetails";
import type { IReference } from "./Interfaces/IReferenceDetails";
...
const MessagingHubSlice = createSlice({
    ...
    reducers: {
        ...
        setReloadedMessages(state, action: PayloadAction<ConversationMessagesDetails>) {
            ...
            const arr = messagesDetails.messages.concat(conv.messages);
            conv.messages = arr;
        },
        ...
        resetMessages(state, action: PayloadAction<string>) {
            let conversation = state.conversations.find((conv) => conv.id === action.payload);
        ...
        deleteAttachmentFromReferences(state, id: PayloadAction<string>) {
            state.draftMessage!.references = state.draftMessage!.references.filter((item) => {
