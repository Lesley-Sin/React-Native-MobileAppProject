...
import { faFile, faTrash } from "@fortawesome/pro-light-svg-icons";
...
export const AttachFilePreview: FC<IAttachFileProps> = ({
  filename,
  uri,
  deliteFile,
  type,
  ...
  const deleteButton = () => {
    return (
      <IconButton
        ...
        icon={<FontAwesomeIcon icon={faTrash} size={30} color="red" />}
      />
    );
  };
  ...
  const defaultFileIcon = () => {
    return <FontAwesomeIcon icon={faFile} size={35} color="blue" />;
  };
  ...
  if (docType === Doctype.Image) {
    return (
      <View style={DocStyles.container}>
        <TouchableHighlight
          onPress={() =>
            navigation.navigate("imagePreview", { uri: uri, type: type })
          }
        >
          <Image source={{ uri: uri }} style={DocStyles.img} />
        </TouchableHighlight>
        {fileName()}
        {deleteButton()}
      </View>
    ...
    return (
      <View style={DocStyles.container}>
        <TouchableHighlight
          onPress={() => navigation.navigate("videoPreview", { uri: uri })}
        >
          {defaultFileIcon()}
        </TouchableHighlight>
        {fileName()}
    ...
    return (
      <View style={DocStyles.container}>
        <Text>Unidentified file type</Text>
      </View>
    );
};
